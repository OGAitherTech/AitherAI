...
function newChat() {
  const c = { id: crypto.randomUUID ? crypto.randomUUID() : String(Date.now()), title: 'New chat', messages: [] };
  chats.unshift(c); activeId = c.id; save(); render(); renderHistory(); input.focus(); sidebar.classList.remove('open');
}
function ensureChat() { if (!active()) newChat(); return active(); }
function titleFor(text) { return text.trim().replace(/\s+/g, ' ').slice(0, 38) || 'New chat'; }

function bubble(role, text) {
  const row = document.createElement('div'); row.className = `msg ${role}`;
  const av = document.createElement('div'); av.className = 'avatar'; av.textContent = role === 'user' ? '🙂' : 'A';
  const wrap = document.createElement('div'); wrap.className = 'message-wrap';
  const name = document.createElement('div'); name.className = 'sender'; name.textContent = role === 'user' ? 'You' : 'Aither';
  const b = document.createElement('div'); b.className = 'bubble'; b.textContent = text;
  wrap.append(name, b); row.append(av, wrap); chat.append(row); return row;
}
function render() {
  const c = active(); chat.innerHTML = ''; const msgs = c?.messages || [];
...
function renderHistory() {
  history.innerHTML = '';
  chats.forEach(c => {
    const button = document.createElement('button'); button.className = `history-item ${c.id === activeId ? 'active' : ''}`;
    button.textContent = c.title || 'New chat'; button.onclick = () => { activeId = c.id; save(); render(); renderHistory(); sidebar.classList.remove('open'); };
...
async function send(text) {
  ...
  if (c.messages.length === 1) c.title = titleFor(text);
  save(); render(); renderHistory(); input.value = ''; resize();
  const sendButton = form.querySelector('.send'); sendButton.disabled = true; input.disabled = true;
  const loader = typing();
  try {
    const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: c.messages }) });
    const data = await response.json(); loader.remove();
    if (!response.ok) throw new Error(data.error || 'Aither request failed.');
    c.messages.push({ role: 'assistant', content: data.reply }); save(); render();
    if ('speechSynthesis' in window) { speechSynthesis.cancel(); speechSynthesis.speak(new SpeechSynthesisUtterance(data.reply)); }
  } catch (error) {
    loader.remove(); c.messages.push({ role: 'assistant', content: `⚠️ ${error.message}` }); save(); render();
...
clear.onclick = newChat; newChatSide.onclick = newChat;
