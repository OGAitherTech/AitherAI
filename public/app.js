...
function save() {
  ...
  if (activeId) localStorage.setItem('aither_active', activeId);
}
...
function bubble(role, text) {
  ...
  const name = document.createElement('div'); name.className = 'sender'; name.textContent = role === 'user' ? 'You' : 'Aither AI';
  const b = document.createElement('div'); b.className = 'bubble';
  b.textContent = text;
  wrap.append(name, b); row.append(av, wrap); chat.append(row);
  return row;
}
function render() {
...
function renderHistory() {
  ...
  chats.forEach(c => {
    ...
    button.onclick = () => { activeId = c.id; save(); render(); renderHistory(); sidebar.classList.remove('open'); };
    history.append(button);
  });
}
function resize() { input.style.height = 'auto'; input.style.height = Math.min(input.scrollHeight, 170) + 'px'; }
function typing() {
  const row = bubble('assistant', ''); row.classList.add('typing-row');
  row.querySelector('.bubble').innerHTML = '<span></span><span></span><span></span>';
  return row;
}
function setBusy(value) {
  busy = value;
  form.querySelector('.send').disabled = value;
...
async function send(text) {
  ...
  input.value = ''; resize(); setBusy(true);
  const loader = typing();
  try {
    ...
    loader.remove();
    if (!response.ok) throw new Error(data.error || `Aither returned HTTP ${response.status}.`);
    c.messages.push({ role: 'assistant', content: data.reply });
    save(); render();
  } catch (error) {
    loader.remove();
    c.messages.push({ role: 'assistant', content: `⚠️ ${error.message}` });
    save(); render();
  } finally {
...
theme.onclick = () => {
  ...
  theme.textContent = dark ? '☀️' : '🌙'; localStorage.setItem(themeKey, dark ? 'dark' : 'light');
};
...
else { if (!active()) activeId = chats[0].id; save(); render(); renderHistory(); }
resize();
